...
Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList2> CommandQueue::CreateCommandList(Microsoft::WRL::ComPtr<ID3D12CommandAllocator> commandAllocator)	
{
	...
	ThrowIfFailed(m_d3d12Device->CreateCommandList(0, m_CommandListType, commandAllocator.Get(), nullptr, IID_PPV_ARGS(&commandList)));
...
uint64_t CommandQueue::ExecuteCommandList(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList2> commandList)
{
	...
	//The ownership of the command alloactor has been transfered to the ComPtr in the command allocator queue.
	//It is safe to realse the reference in this temporary COM pointer here.
