...
//avoid conflicts with min and max functions defined in <algorithm>
#if defined(min)
#undef min
#endif
...
//a function named CreateWindow will be presented in the future
...
//must be included before d3d12.h
...
//window rectangle (prev windows dimensions before going fullscreen)
RECT g_WindowRect;
...
{
	//Registre a window class for creating our render window with
...
HWND CreateWindow(const wchar_t* windowClassName, HINSTANCE hInst, const wchar_t* windowTitle, uint32_t width, uint32_t height)
{
	...
	int windowHeight = windowRect.bottom - windowRect.top;	//sus
...
ComPtr<IDXGISwapChain4> CreateSwapChain(HWND hWnd, ComPtr<ID3D12CommandQueue> commandQueue, uint32_t width, uint32_t height, uint32_t bufferCount)
{
	...
	//It is recommended to always allow tearing if tearing support is avaliable
...
//event to be waited by the CPU until fence reaches desired value
...
//TODO: review why 'fenceValueForSignal' is incremented before signaling on the command queue
...
void Update()
{
	...
	if (elapsedSeconds > 1.0)
	{
		...
		sprintf_s(buffer, bufferSize, "FPS: %f\n", fps);
...
//TODO: Fullscreen and Window Resize
...
int CALLBACK wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nCmdShow)
{
	...
	const wchar_t* windowClassName = L"JayRenderWindow";
	...
	g_hWnd = CreateWindow(windowClassName, hInstance, L"JayRender3D12", g_ClientWidth, g_ClientHeight);

	//Initialize the global window rect variable.
	::GetWindowRect(g_hWnd, &g_WindowRect);

#pragma region DX12_OBJECTS_INITIALIZATION
